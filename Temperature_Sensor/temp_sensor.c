...
#define ADC0_Powerdown_mode_off (1<<21)
...
{
	while((U0LSR&(1<<5))==0);//Wait till Bit 5 - Transmitter Holding Register Empty (THRE) bit becomes 1 which tells that previous transmission is completed
	U0THR = ch;
	while((U0LSR&(1<<5))==0);	//Wait till Bit 5 - Transmitter Holding Register Empty (THRE) bit becomes 1 which tells that transmission is completed
}
...
void ADC_Init() {
    PINSEL1 = (1<<26); /* P0.29 as AD0.1 */
		AD0CR = ADC0_2 | ADC_DIVIDER | ADC0_Powerdown_mode_off; /* ADC operational on 0.29 (ADC 0.2), 10-bits, 11 clocks for conversion */
}

unsigned int ADC_Read() {
    unsigned int result;
		AD0CR = AD0CR | (1<<24); /* Start Conversion */
		while ( !(AD0DR2 & 0x80000000) ); /* Wait till DONE */
		result = AD0DR2;
		result = (result>>6);
...
int main() {
    ...
    while(1) {
				...
				float temp = ((float)adc_val * VREF * 100)/1024; //As per the Equation given in the tutorial
			
        char msg[50];
				sprintf(msg,"Temp = %0.1f Deg. Celsius\n" , temp);
